module icache_direct_3(
    ...
    parameter MIDDLE = 1;            // 中间状态
    parameter READ_SRAM = 2;         // 读取 SRAM 状态
    ...
    // 状态机状态寄存器
    reg[1:0] state, next_state, finish_read;
    ...
    always@(posedge clk or posedge rst) begin
        if(rst) begin
            ...
            state <= next_state;
            if(state == READ_SRAM || state == MIDDLE) begin
                finish_read <= finish_read + 1;  // 读取完成标志计数
            end else begin
                finish_read <= 2'b00;
            end
        end
